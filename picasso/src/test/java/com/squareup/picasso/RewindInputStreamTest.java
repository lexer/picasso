...
public class RewindInputStreamTest {
  @Test public void oneReadBuffered() throws IOException {
    ...
    RewindInputStream ris = new RewindInputStream(in);
  ...
  @Test public void skipBeforeRewindBuffers() throws IOException {
    ByteArrayInputStream in = new ByteArrayInputStream(bytes("FOOBARBAZ"));
    RewindInputStream ris = new RewindInputStream(in);
  ...
  @Test public void skipCrossingBufferBoundary() throws IOException {
    ByteArrayInputStream in = new ByteArrayInputStream(bytes("FOOBAR"));
    RewindInputStream ris = new RewindInputStream(in);
    ...
    ris.read(buffer2);
    assertBytes(buffer2).isEqualTo("FO");

    // Buffer has one byte left here. Just skip what is there.
    assertThat(ris.skip(2)).isEqualTo(1);
    // No buffer. Delegate down to the real stream.
    assertThat(ris.skip(1)).isEqualTo(1);

    ris.read(buffer2);
    assertBytes(buffer2).isEqualTo("AR");
  }
